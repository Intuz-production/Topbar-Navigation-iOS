...
- (NSDateComponents *)dateComponentsForType:(NSCalendarUnit)calanderUnit ToDate:(NSDate *)anotherDate {
    NSDate *fromDate;
    NSDate *toDate;
    ...
    NSDateComponents *difference = [calendar components:calanderUnit
                                               fromDate:fromDate toDate:toDate options:0];
    return difference;
}

- (NSInteger) differenceInDay:(NSDate *)anotherDate
{
    
    return [[self dateComponentsForType:NSCalendarUnitDay ToDate:anotherDate] day];
}

- (NSInteger) differenceInMonth:(NSDate *)anotherDate
{
    return [[self dateComponentsForType:NSCalendarUnitMonth ToDate:anotherDate] month];
}

- (NSInteger) differenceInYear:(NSDate *)anotherDate;
{
    return [[self dateComponentsForType:NSCalendarUnitYear ToDate:anotherDate] year];;
}
...
- (double)get12HourTimeWithTimeZone:(NSString *)strTimeZone {
    NSTimeZone *timeZone = [NSTimeZone timeZoneWithName:strTimeZone];
    ...
    if (time.length==8) {
        NSString *hour = [time substringWithRange:NSMakeRange(0, 5)];
        double hm = [[hour stringByReplacingOccurrencesOfString:@":" withString:@"."] doubleValue];
        return hm;
    }
    return 0.0;
}

- (double)get24HourMinWithTimeZone:(NSString *)strTimeZone {
    NSTimeZone *timeZone = [NSTimeZone timeZoneWithName:strTimeZone];
    NSDateFormatter *df = [[NSDateFormatter alloc]init];
    [df setTimeZone:timeZone];
    [df setDateFormat:@"HH"];
    NSString *time = [df stringFromDate:self];
    double min = [time integerValue]*60;
    [df setDateFormat:@"mm"];
    time = [df stringFromDate:[NSDate date]];
    min =  min + [time integerValue];
    return min;
}
...
- (NSString *)dayString {
    ...
    return [[dtFormatter stringFromDate:self] uppercaseString];
}

- (NSDate *)dateWithTimeZone:(NSString *)strTimeZone {
...
- (NSString *)timeIntervalString {
    double time = [self timeIntervalSince1970];
