...
+ (UIColor*) colorWithHexString:(NSString *)stringToConvert
{
    ...
    range.location = startRange+2;
    NSString *gString = [cString substringWithRange:range];
    
    range.location = startRange+4;
    ...
    //    float alpha = 255 * colorAlpha;
    //    float Red = r*(alpha/255) + 255*(1-(alpha/255));
    //    float Green = g*(alpha/255) + 255*(1-(alpha/255));
    //    float Blue = b*(alpha/255) + 255*(1-(alpha/255));
    
    //    NSLog(@" R %f G %f B %f of %@",Red,Green,Blue, stringToConvert);
    
    
    return color;
}

+ (NSString *)hexCodeFromAlpha:(CGFloat)percentage {
    int alpha = (int) round(percentage * 255);
    NSString *hex = [[NSString stringWithFormat:@"%lX",
                      (unsigned long)alpha] uppercaseString];
    if ([hex length] == 1) hex = [NSString stringWithFormat:@"0%@", hex];
    return hex;
}
...
+ (UIColor *) colorWithHexStringReturnRGB:(NSString *)stringToConvert
{
    ...
    // String should be 6 or 8 characters
    if ([cString length] < 6)
        return [UIColor whiteColor];// @"1.00,1.00,1.00";
    
    // strip 0X if it appears
    if ([cString hasPrefix:@"0X"]) cString = [cString
...
//    NSString *strRGB=[NSString 
//                      stringWithFormat:@"%.2f,%.2f,%.2f",Red,Green,Blue];
//    
//   
...
+ (UIColor*)gradientFromColors:(NSArray *)arrColors withHeight:(int)height
{
    ...
    for (UIColor *cl in arrColors) {
        [colors addObject:(id)cl.CGColor];
    }
...
{
    return UICOLOR_FROM_RGB_VALUE(0xE74C3C);
}
+ (UIColor *)flatDarkRedColor
{
    return UICOLOR_FROM_RGB_VALUE(0xC0392B);
}

#pragma mark - Green
+ (UIColor *)flatGreenColor
{
    return UICOLOR_FROM_RGB_VALUE(0x2ECC71);
}
+ (UIColor *)flatDarkGreenColor
{
    return UICOLOR_FROM_RGB_VALUE(0x27AE60);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0x3498DB);
}
+ (UIColor *)flatDarkBlueColor
{
    return UICOLOR_FROM_RGB_VALUE(0x2980B9);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0x1ABC9C);
}
+ (UIColor *)flatDarkTealColor
{
    return UICOLOR_FROM_RGB_VALUE(0x16A085);
}

#pragma mark - Purple
+ (UIColor *)flatPurpleColor
{
    return UICOLOR_FROM_RGB_VALUE(0x9B59B6);
}
+ (UIColor *)flatDarkPurpleColor
{
    return UICOLOR_FROM_RGB_VALUE(0x8E44AD);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0xF1C40F);
}
+ (UIColor *)flatDarkYellowColor
{
    return UICOLOR_FROM_RGB_VALUE(0xF39C12);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0xE67E22);
}
+ (UIColor *)flatDarkOrangeColor
{
    return UICOLOR_FROM_RGB_VALUE(0xD35400);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0x95A5A6);
}

+ (UIColor *)flatDarkGrayColor
{
    return UICOLOR_FROM_RGB_VALUE(0x7F8C8D);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0xECF0F1);
}

+ (UIColor *)flatDarkWhiteColor
{
    return UICOLOR_FROM_RGB_VALUE(0xBDC3C7);
}
...
{
    return UICOLOR_FROM_RGB_VALUE(0x34495E);
}

+ (UIColor *)flatDarkBlackColor
{
    return UICOLOR_FROM_RGB_VALUE(0x2C3E50);
}
